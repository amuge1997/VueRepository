...
let selfShowConpoent={
    ...
    mounted:function(){
        for(let i=0;i<this.arrLen;i++){
            this.$refs.li[i].style.top=(i * 90).toString() + 'px'
        }
    },
    methods:{
        updateLi:function () {
            ...
            setTimeout(function () {
                ...
                templi.offsetWidth;         // 立即重绘

                that.$refs.li[hideIndex].style.transition='1s'
                that.$refs.templi.style.transition='1s'
                if(hideIndex===0){that.updatePara.hideIndex=that.arrLen-1}
                else {that.updatePara.hideIndex-=1}
                that.updatePara.lock=true
            },time)
        },
        showAndHideDate:function (index,showOrHide) {
    ...
    watch:{
        newdata:function (val,oldVal) {
            this.updatePara.newData=val
            this.updateLi()
        }
    },
    template: '<div id="div-show" ref="main">\n' +
        ...
        '            <div   ref="li" class="div-show-li" ' +
        '                   v-for="(v,i) in arr" ' +
        '                   @mouseover="showAndHideDate(i,\'show\')" ' +
        '                   @mouseleave="showAndHideDate(i,\'hide\')">\n' +
...
var app=new Vue({
    ...
    methods:{
        inputEmitFunc:function (data) {
            ...
            this.inputData={text:data,time:[month,day,year].join('.')}
        },
    }
})
